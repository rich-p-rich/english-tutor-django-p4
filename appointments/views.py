...
def make_appointment(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                appointment.save()
                request.session['appointment_date'] = \
                    str(appointment.meeting_date)
                request.session['appointment_time'] = \
                    str(appointment.meeting_time)
    ...
    return render(
        request,
        'appointments/book-appointments.html',
        {'form': form}
    )
...
def confirm_appointment(request):
    ...
    return render(
        request,
        'appointments/appointment-confirmed.html',
...
def manage_appointments(request):
    """
    View for managing appointments -> allows user to see all their
    appointments and edit or cancel them
    """
    user = request.user
    ...
    if request.method == 'POST':
        if 'select' in request.POST:
            ...
            appointment_to_edit = get_object_or_404(
            ...
            change_form = ChangeAppointmentForm(
                request.POST, instance=appointment_to_edit
            )
            if change_form.is_valid():
                try:
                    ...
                    return render(
                        request,
                        'appointments/change-confirmed.html',
            ...
            appointment_id = request.POST.get('appointment_id')
            appointment_to_cancel = get_object_or_404(
                Appointment, id=appointment_id
            )
            appointment_to_cancel.delete()
            return render(
                request,
                'appointments/cancellation-confirmed.html',
                {'appointment': appointment_to_cancel}
            )

    return render(
        request,
        'appointments/change-or-cancel.html',
        {
